...
namespace RenderJoy
{
    ...
    void RenderJoyTrianglePass::LoadShader()
    {
        ...
        // Load Pass SRG
        const auto srgLayout = m_shader->FindShaderResourceGroupLayout(AZ::RPI::SrgBindingSlot::Pass);
        if (srgLayout)
        {
            m_shaderResourceGroup = AZ::RPI::ShaderResourceGroup::Create(m_shader->GetAsset(), m_shader->GetSupervariantIndex(), srgLayout->GetName());
        ...
        m_stencilRef = aznumeric_caster(passData->m_stencilRef);
    ...
    void RenderJoyTrianglePass::CreateImageForChannel(uint32_t imageChannelIdx, const AZStd::string& imageName, size_t width, size_t height, uint32_t color)
    {
        ...
        const uint32_t pixelDataSize = aznumeric_cast<uint32_t>(width * height * sizeof(color));
